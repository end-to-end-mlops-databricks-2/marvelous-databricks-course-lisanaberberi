...
def create_or_refresh_monitoring(config, spark, workspace):
    ...
    inf_table = spark.sql(f"SELECT * FROM {config.catalog_name}.{config.schema_name}.`model-serving-fe_payload_payload`")
    ...
    df_final_with_status = df_final \
        .join(test_set.select("Id", "SalePrice"), on="Id", how="left") \
        ...
        .select(
            "*",  
            F.coalesce(F.col("sale_price_test"), F.col("sale_price_inference")).alias("sale_price")
        ) \
    ...
    df_final_with_features = df_final_with_status \
        .join(house_features, on="Id", how="left")
